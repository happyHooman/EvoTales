...
from config import SMARTIE_CONFIG
...
class Smarty(Entity):
    ...
    def spawn(cls, scene, map_size, x: float | None = None, y: float | None = None):
        """Handler: create and register a smartie at (x, y). Defaults to map center."""
    ...
    def spawn_initial(cls, scene, map_size):
        """Spawn initial smartie population according to config."""
        padding = SMARTIE_CONFIG.get("bounds_padding", 20)
        map_w, map_h = map_size
        for _ in range(SMARTIE_CONFIG["initial_count"]):
            x = random.uniform(padding, map_w - padding)
            y = random.uniform(padding, map_h - padding)
            cls.spawn(scene, map_size, x, y)

    def __init__(self, x, y, scene):
        texture = sprite_manager.get_texture("smartie")
        super().__init__(texture, x, y)
        self.scene = scene
        self.config = SMARTIE_CONFIG
