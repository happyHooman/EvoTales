...
class Carnivore(Entity):
    ...
    def spawn_initial(cls, scene, map_size):
        """Spawn initial carnivore population according to config."""
        padding = CARNIVORE_CONFIG.get("bounds_padding", 20)
        map_w, map_h = map_size
        for _ in range(CARNIVORE_CONFIG["initial_count"]):
            x = random.uniform(padding, map_w - padding)
            y = random.uniform(padding, map_h - padding)
            cls.spawn(scene, map_size, x, y)

    def __init__(self, x, y, scene):
        texture = sprite_manager.get_texture("carnivore")
        super().__init__(texture, x, y)
        self.scene = scene
        self.config = CARNIVORE_CONFIG
